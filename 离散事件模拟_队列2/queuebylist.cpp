...
void time(int i);
int bestwindow(queuelist windows[], int nWin);//自动选择最好的窗口

void simulate(int nWin, int servTime)
{

	queuelist* windows = new queuelist[nWin];
	for (int i = 0; i < servTime; i++)
	{
		...
		for (int i = 0; i < nWin; i++) {
			if (windows[i].size() > 0) {
				windows[i].frontdec();
				if ((windows[i].getfront()->succ->time -1) <= 0)		//这里getfront就是顾客时间
					windows[i].dequeue();
			}
		}
		time(i);
...
int bestwindow(queuelist windows[],int nWin)
{
